//Your WIFI data
const char* ssid     = "Menna";   //your network SSID
const char* password = "menna12345";   //your network password

// tesseract is the library responsible for text recognition
// It is connected via cloud 
...
#include "soc/rtc_cntl_reg.h"   // real time clock

String Feedback="";   //Return Client Message
//Command parameter values
String Command="",cmd="",P1="",P2="",P3="",P4="",P5="",P6="",P7="",P8="",P9=""; // needed parameters for the esp32 camera.h 
// Command parsing status values
byte ReceiveState=0,cmdState=1,strState=1,questionstate=0,equalstate=0,semicolonstate=0; // needed pararmeters for the esp32 camera.h

// WARNING!!! Make sure that you have either selected ESP32 Wrover Module,
//            or another board which has PSRAM enabled

// Trustworthy ESP32-CAM module pin settings
...
void ExecuteCommand() //بتوصلهم ببعض 
{
  //Serial.println("");
  ...
  if (cmd=="your cmd") {
    ...
    Feedback="AP IP: "+WiFi.softAPIP().toString();    // دا الاي بي اللي بناخده كوبي ونرفعه علي البراوزر
    Feedback+=", ";
    Feedback+="STA IP: "+WiFi.localIP().toString(); // دا الايبي بتاع الجهاز نفسه 
  }  
  else if (cmd=="mac") {
    ...
    Serial.print("Connecting to "); // this is the part where the esp32 cam module connects to wifi
    Serial.println(P1);
...
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  //Disable power unstable
...
static const char PROGMEM INDEX_HTML[] = R"rawliteral(
  <!DOCTYPE html>
  <head>
  <title>Text recognition Gharbia</title>
  <meta charset="utf-8">
...
void loop() {
  ...
  if (client) { 
    ...
    while (client.connected()) {
      if (client.available()) {
        char c = client.read();             
        // كومنت هنا علشان نعرف ان السي دي بتاعت التواصل ما بين كل حاجه علشان تعرف تقولي انا غلطان في اي باستخدام حرف واحد علشان اسرع العملية شوية 
        // دي بقا ال سي دي تبع المكتبه بتاعت ال اي اس بي كاميرا 
        getCommand(c);   
